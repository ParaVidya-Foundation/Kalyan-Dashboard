...
export default function NotFound() {
  ...
  return (
    <main className="relative min-h-[92dvh] w-full overflow-hidden bg-[#0b0d12] text-white">
      {/* Background visual */}
      <div
        className="pointer-events-none absolute inset-0 -z-10 [background:radial-gradient(1200px_600px_at_50%_10%,rgba(255,26,26,0.15),transparent_60%),radial-gradient(1000px_500px_at_0%_100%,rgba(99,102,241,0.15),transparent_60%)]"
        aria-hidden
      />
      ...
      <section aria-label="Page not found" className="relative mx-auto flex max-w-7xl flex-col items-center px-6 py-20 sm:py-28 md:py-32">
        ...
        <motion.h1
          ...
          className="bg-gradient-to-b from-white to-white/60 bg-clip-text text-center text-6xl font-extrabold leading-none text-transparent sm:text-7xl md:text-8xl"
        >
          404
        </motion.h1>
        ...
        >
          <Link
            href="/"
            className="inline-flex items-center gap-2 rounded-xl bg-white px-5 py-3 text-sm font-semibold text-gray-900 shadow-[0_0_24px_rgba(255,255,255,0.2)] transition hover:brightness-95"
          >
            Go Home
          ...
          <Link
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-[#FF1A1A] px-5 py-3 text-sm font-semibold text-white shadow-[0_0_24px_#FF1A1A] transition hover:brightness-110"
          >
            Explore AI Blogs
          </Link>
        </motion.div>
        ...
        <motion.div
          ...
          className="mt-12 grid w-full max-w-4xl grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3"
        >
          {links.map((l) => {
            const Icon = l.icon;
            return (
              <Link
                key={l.href}
                href={l.href}
                className="group relative overflow-hidden rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur-sm transition hover:bg-white/10"
              >
