...
import AstroProfileCard, { AstroProfileProps } from "../../cards/AstroProfileCard";
...
export function KundliDashboard() {
  const [data, setData] = useState<AstroProfileProps[]>(initialData);
  const [query, setQuery] = useState("");
  const [selectedTag, setSelectedTag] = useState<string | null>(null);
  const [sortBy, setSortBy] = useState<"date" | "name" | "rashi">("date");

  const [editing, setEditing] = useState<null | { id: string; payload: Record<string, string> }>(null);
  ...
  function onSave(updated: Record<string, any>) {
    setData((prev) =>
      prev.map((d) =>
        d.id === updated.id
          ? {
              ...d,
              ...updated,
              tags: typeof updated.tags === "string" ? updated.tags.split(",").map((s: string) => s.trim()) : updated.tags,
            }
          : d
      )
    );
  ...
  return (
    <DashboardShell
      title="Kundli Cloud"
      description="Search, sort, and curate every saved chart in one calm space."
    >
