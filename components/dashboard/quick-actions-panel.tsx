...
export function QuickActionsPanel({ title = "Quick Actions", actions }: QuickActionsPanelProps) {
  return (
    <motion.aside
      ...
      className="rounded-3xl border border-white/60 bg-white/80 p-5 shadow-2xl shadow-indigo-100 backdrop-blur-xl"
    >
      <p className="text-xs font-semibold uppercase tracking-[0.3em] text-gray-400">{title}</p>
      <div className="mt-4 space-y-4">
        {actions.map((action) => (
          <div key={action.label} className="rounded-2xl border border-gray-100/70 bg-gray-50/60 p-4">
            ...
            <Button
              variant="ghost"
              size="sm"
              className="mt-3 rounded-full px-3 text-gray-700 hover:bg-white"
