...
interface DivisionalChartHeaderProps {
  title: string;
  subtitle: string;
}

export function DivisionalChartHeader({ title, subtitle }: DivisionalChartHeaderProps) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.6, ease: [0.19, 1, 0.22, 1] }}
      className="mb-12 sm:mb-16 will-change-transform-opacity"
    >
