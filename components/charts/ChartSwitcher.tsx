...
export function ChartSwitcher({ chart }: ChartSwitcherProps) {
  ...
  const chartImages: Record<"north" | "south", { src: string; alt: string }> = {
    north: {
      src: "/North.png",
    ...
    <div className="mb-16">
      ...
      <div className="relative bg-gradient-to-br from-yellow-50/30 via-white to-yellow-50/20 rounded-3xl border border-yellow-100/80 p-6 sm:p-12 shadow-[0_8px_32px_rgba(255,244,194,0.12)] min-h-[500px] flex items-center justify-center overflow-hidden">
        <AnimatePresence mode="wait" initial={false}>
          ...
          >
            <OptimizedImage
              src={chartImages[chartStyle].src}
              alt={chartImages[chartStyle].alt}
              width={900}
              height={900}
              className="max-w-[520px] w-full rounded-2xl border border-yellow-100/80 shadow-[0_12px_45px_rgba(15,23,42,0.08)]"
              quality={90}
              priority={chartStyle === "north"}
            />
          </motion.div>
        </AnimatePresence>
      </div>
