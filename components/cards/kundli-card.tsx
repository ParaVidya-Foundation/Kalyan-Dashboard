...
export function KundliCard({ title, chart, layer, updatedAt, energy, accentClass }: KundliCardProps) {
  return (
    <motion.div
      whileHover={{ y: -10, opacity: 0.98 }}
      transition={{ duration: 0.35, ease: [0.16, 1, 0.3, 1] }}
      className={cn(
        "group rounded-3xl border border-white/60 bg-white/80 p-5 shadow-lg shadow-indigo-50 backdrop-blur-xl",
        accentClass
      )}
    >
