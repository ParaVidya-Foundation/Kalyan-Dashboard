...
export function DocumentCard({ title, type, tags, updatedAt, preview}: DocumentCardProps) {
  return (
    <motion.article
      whileHover={{ y: -10, opacity: 0.98 }}
      transition={{ duration: 0.35, ease: [0.16, 1, 0.3, 1] }}
      className="flex h-full flex-col rounded-3xl border border-white/60 bg-white/80 p-5 shadow-lg shadow-indigo-50 backdrop-blur-xl"
    >
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {tags.map((tag) => (
          <Badge key={tag} variant="outline" className="rounded-full border-gray-200 text-gray-600">
            {tag}
          </Badge>
        ))}
      </div>
