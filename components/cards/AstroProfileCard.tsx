    ...
    >
      ...
      <div className="flex items-center gap-4">
        <div className="h-14 w-14 shrink-0 overflow-hidden rounded-full bg-yellow-50 ring-1 ring-yellow-100">
          ...
          <img
            src="/mnt/data/f84134c0-8298-491e-8734-35d8f54d2d8b.png"
            alt={`${name} avatar`}
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {tags.length ? (
          tags.map((t) => (
            <span
              key={t}
              className="rounded-full px-3 py-1 text-xs bg-yellow-50 text-yellow-800 border border-yellow-100"
            >
              {t}
            </span>
          ))
      ...
      <div className="mt-4 flex items-center justify-between gap-3">
        <button
          onClick={() => onView?.(id)}
          className="inline-flex items-center gap-2 rounded-lg bg-white px-4 py-2 text-sm font-medium text-gray-900 border border-yellow-100 shadow-sm hover:translate-y-[-2px] transition-transform"
        >
          View
        </button>
        ...
        <button
          onClick={() => onEdit?.(id)}
          className="inline-flex items-center gap-2 rounded-lg bg-yellow-400 px-4 py-2 text-sm font-medium text-white shadow hover:translate-y-[-2px] transition-transform"
        >
          Edit
        </button>
      </div>
