...
export default function MatchMaking({
  ...
  const handlePlaceChange = (value: string) => {
    ...
    setSuggestions(
      value.length > 2 ? [`${value} City`, `${value} District`, `${value} Country`] : []
    );
  };
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          {(errors.hour || errors.minute) && (
            <p className="error-text">Please select valid birth time</p>
          )}
        </div>
        ...
        <div className="space-y-2 relative">
          ...
          {suggestions.length > 0 && (
            <div className="absolute w-full bg-white/80 backdrop-blur-md border rounded-lg shadow-lg z-20 mt-1">
              {suggestions.map((s, i) => (
                <div
                  key={i}
